...
const JenresList: React.FC<JenresListProps> = ({ jenres, isJenreLoading }) => {
  ...
  return (
    <div className="sidebar flex flex-row flex-wrap gap-x-1.5">
      {jenres.map((jenre) => (
        <div
          key={jenre.id}
          className="bg-[#1E1E1E] text-white mx-1 my-1 rounded-lg px-2 py-1 cursor-pointer"
          onClick={() => handleClick(jenre.id)}
        >
          <h3 className="text-xl font-bold">{jenre.russian}</h3>
        </div>
      ))}
    </div>
  );
